...
function Setcom() {
  ...
  const [productDescription9, setProductDescription9] = useState('');
  const [productDescription10, setProductDescription10] = useState('');
  ...
  const handleAddProduct = () => {
    fetch('http://localhost:3000/fullset')
      .then((response) => response.json())
      .then((data) => {
        const highestId = data.reduce((maxId, product) => {
          const productIdNumber = Number(product.id.replace('F', ''));
          return productIdNumber > maxId ? productIdNumber : maxId;
        }, 0);

        const product = {
          id: `F${String(highestId + 1).padStart(2, '0')}`,
          name: productName,
          ...
          description9: productDescription9,
          description10: productDescription10,
          price: productPrice,
          image: productImage,
        };

        return fetch('http://localhost:3000/fullset', {
          method: 'POST',
          headers: {
      ...
      .then((data) => {
        ...
        setProductDescription9('');
        setProductDescription10('');
        setProductPrice('');
        setProductImage('');
      })
      .catch((error) => {
    ...
    <div>
      ...
      {showAddProductModal && (
        ...
        <div className="add-product-modal">
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <div>
          <label htmlFor="productDescription9">Product Description9:</label> <br />
          <textarea
            id="productDescription9"
            value={productDescription9}
            onChange={(e) => setProductDescription9(e.target.value)}
          /></div>
            <div>
              <label htmlFor="productDescription10">Product Description10:</label> <br />
              <textarea
                id="productDescription10"
                value={productDescription10}
                onChange={(e) => setProductDescription10(e.target.value)}
              />
            </div>
            </div>
          <br /> <br />
